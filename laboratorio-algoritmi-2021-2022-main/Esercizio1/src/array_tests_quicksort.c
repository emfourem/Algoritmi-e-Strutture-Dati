...
static void test__one_elem_string(void)
{
  record *exp_arr[] = {&i1};
  array_add(_array, &i1);
  quick_sort(_array, precedes3cases_record_field1);
...
static void test__one_elem_int(void)
{
  record *exp_arr[] = {&i2};
  array_add(_array, &i2);
  quick_sort(_array, precedes3cases_record_field2);
  record **act_arr = malloc(1 * sizeof(record *));
  for (unsigned long i = 0; i < 1; i++)
    act_arr[i] = (record *)array_get(_array, i);
  TEST_ASSERT_EQUAL_PTR_ARRAY(exp_arr, act_arr, 1);
...
static void test__one_elem_float(void)
{
  record *exp_arr[] = {&i1};
  array_add(_array, &i1);
  quick_sort(_array, precedes3cases_record_field3);
  record **act_arr = malloc(1 * sizeof(record *));
  for (unsigned long i = 0; i < 1; i++)
    act_arr[i] = (record *)array_get(_array, i);
  TEST_ASSERT_EQUAL_PTR_ARRAY(exp_arr, act_arr, 1);
